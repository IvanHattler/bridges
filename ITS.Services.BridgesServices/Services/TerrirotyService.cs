...
namespace ITS.Services.BridgesServices.Services
{
    ...
    public class TerritoryService : AbstractService<Territory, long>, ITerritoryService
    {
        protected override IDAO<Territory, long> GetDAO()
        {
            try
            {
                return ApplicationService.GetDaoService().GetDao<ITerritoryDAO>();
            }
            catch (Exception ex)
            {

...
namespace ITS.Services.BridgesServices.Services
{
    ...
    public class DocumentationInfoService : AbstractService<DocumentationInfo, long>, IDocumentationInfoService
    {
        protected override IDAO<DocumentationInfo, long> GetDAO()
        {
            try
            {
                return ApplicationService.GetDaoService().GetDao<IDocumentationInfoDAO>();
            }
            catch (Exception ex)
            {

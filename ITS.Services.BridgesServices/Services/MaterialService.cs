...
namespace ITS.Services.BridgesServices.Services
{
    ...
    public class MaterialService : AbstractService<Material, long>, IMaterialService
    {
        protected override IDAO<Material, long> GetDAO()
        {
            try
            {
                return ApplicationService.GetDaoService().GetDao<IMaterialDAO>();
            }
            catch (Exception ex)
            {

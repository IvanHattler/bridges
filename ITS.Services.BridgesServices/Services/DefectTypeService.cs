...
namespace ITS.Services.BridgesServices.Services
{
    ...
    public class DefectTypeService : AbstractService<DefectType, long>, IDefectTypeService
    {
        protected override IDAO<DefectType, long> GetDAO()
        {
            try
            {
                return ApplicationService.GetDaoService().GetDao<IDefectTypeDAO>();
            }
            catch (Exception ex)
            {

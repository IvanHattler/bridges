...
namespace ITS.Services.BridgesServices.Services
{
    ...
    public class TypicalProjectService : AbstractService<TypicalProject, long>, ITypicalProjectService
    {
        protected override IDAO<TypicalProject, long> GetDAO()
        {
            try
            {
                return ApplicationService.GetDaoService().GetDao<ITypicalProjectDAO>();
            }
            catch (Exception ex)
            {

...
namespace ITS.Services.BridgesServices.Services
{
    ...
    public class BridgeObstacleService : AbstractService<BridgeObstacle, long>, IBridgeObstacleService
    {
        protected override IDAO<BridgeObstacle, long> GetDAO()
        {
            try
            {
                return ApplicationService.GetDaoService().GetDao<IBridgeObstacleDAO>();
            }
            catch (Exception ex)
            {

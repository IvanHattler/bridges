...
namespace ITS.Services.BridgesServices.Services
{
    ...
    public class SpanStructureService : AbstractService<SpanStructure, long>, ISpanStructureService
    {
        protected override IDAO<SpanStructure, long> GetDAO()
        {
            try
            {
                return ApplicationService.GetDaoService().GetDao<ISpanStructureDAO>();
            }
            catch (Exception ex)
            {

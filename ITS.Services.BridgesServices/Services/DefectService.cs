...
namespace ITS.Services.BridgesServices.Services
{
    ...
    public class DefectService : AbstractService<Defect, long>, IDefectService
    {
        protected override IDAO<Defect, long> GetDAO()
        {
            try
            {
                return ApplicationService.GetDaoService().GetDao<IDefectDAO>();
            }
            catch (Exception ex)
            {
